...
url_humidity = ['https://www.tutiempo.net/sant-feliu-de-guixols.html',"https://www.tutiempo.net/begur.html"]
url_sunset =['https://www.sunrise-and-sunset.com/es/sun/espana/sant-feliu-de-guixols','https://www.sunrise-and-sunset.com/es/sun/espana/begur']
...
#   Append list with data  !important maybe delete
def get_data_rewrite(info_test,url,b,a):
    info_test[b][a] = get_data(url) #   Put the html info into list !! this is a test change to get_data(url) 
...
def thread_url(name,url,slices):
    ...
    return info_test
#   get information of the data passed !important maybe delete, no finished
def get_info_slice(data,slices):
    # 0, Temp_windspeed, 1, Humidity, 2, Sunrise_sunset
    for a in range(0,len(data),3):
        print(f"TEMPERATURE :  {get_info(data[a],slices[0][0],slices[0][1])}")
        
           

def test(name,url,slices):
...
#   Main program
test(name,url,slices)
try:
    
    while KeyboardInterrupt:
        print("tracking...")
        print(f"start ... {dati.datetime.now()}")
        info = update_array(name,url,slices)
